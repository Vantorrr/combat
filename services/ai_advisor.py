...
def calculate_financial_analysis(
    ...
    *,
    ...
    system_prompt = (
        ...
        "Кратко перечисли: макс. сумму гарантии, лимит, оценку долгов и капитала. "
        "Сделай финальный вывод о надёжности клиента и потенциале сделки.\n\n"
    ...
    try:
        ...
        return (
            ...
            f"Последний звонок: {last_call_str} — {last_comment}\n\n"
            f"Инфоповоды для звонка:\n"
            f"1. Ошибка генерации (новости недоступны).\n"
            f"2. Ошибка генерации (праздники недоступны).\n"
            f"3. Ошибка генерации (анализ истории недоступен).\n"
            f"4. Ошибка генерации (фин. справка недоступна)."
        )
